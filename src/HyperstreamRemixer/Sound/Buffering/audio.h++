...
class Audio final {
  public:
    ...
    ~Audio();
    void play();
    static auto from_mp3_file(std::initializer_list<AudioEffect *> &&effects, const std::string &file_path, audio_fx_behavior_t fx_behavior = APPLY_FX_NOW) -> Audio *;
  ...
  private:
    auto apply_effects(const Allocation<wf_amplitude_t> &buffer) const -> Allocation<wf_amplitude_t>;
