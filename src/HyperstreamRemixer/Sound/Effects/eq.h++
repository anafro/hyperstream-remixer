...
namespace HyperstreamRemixer::Sound::Effects {
    ...
    [[nodiscard]]
    static constexpr size_t calculate_ascii_scale_length(const std::string& scale) {
        return scale.length();
    }

    [[nodiscard]]
    static constexpr eq_gain_t calculate_ascii_band_gain(const size_t band_max_length, const std::string& band) {
